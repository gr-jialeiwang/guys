...
class NewKoji_index extends NK_Page {
	...
	function init() {
		...
		// タイトル・パーツテンプレート設定
		$this->header_title = "新規工事登録／初期登録情報編集";
	...
	function process() {
		...
		switch ($mode) {
			...
			case "index":
				...
				// メインテンプレート
				$this->contents_tpl = $this->xml->getTemplate("new_koji", "index");
				break;

			case "new":

				// メインテンプレート
				$this->contents_tpl = $this->xml->getTemplate("new_koji", "edit");
...
// 発注先取得
		...
		$sql = $this->xml->getSql("new_koji", "select_orderClient");
		$arrData = $this->selectTable2($sql, array());
		foreach ($arrData as $key => $value) {
			$codeNumber = $value['codeNumber'];
			$officeCode = $value['officeCode'];
			$clientRepresentative1 = $value['clientRepresentative1'];
			$clientRepresentative2 = $value['clientRepresentative2'];
			$clientRepresentative3 = $value['clientRepresentative3'];
			$clientRepresentative4 = $value['clientRepresentative4'];
			$clientRepresentative5 = $value['clientRepresentative5'];
			$office = $value['office'];
			$officialName = $value['officialName'];
			$arrList = array();
			$arrList['masterId'] = $codeNumber."｜".$officeCode;;
			$arrList['name'] = $officialName."｜".$office;;
			$arrList['clientRepresentative1'] = $clientRepresentative1;
			$arrList['clientRepresentative2'] = $clientRepresentative2;
			$arrList['clientRepresentative3'] = $clientRepresentative3;
			$arrList['clientRepresentative4'] = $clientRepresentative4;
			$arrList['clientRepresentative5'] = $clientRepresentative5;
 ...
 // $this->xml->init("index_hara", false);
