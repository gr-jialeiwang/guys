...
class Top extends NK_Page {
	...
	function init() {
		...
		// 権限チェック
		$sql = $this->xml->getSql("common", "status_constructionSearchEdit");
		$queryArr = array();
		...
		// 権限チェック
		$sql = $this->xml->getSql("common", "status_aggregatedDataEdit");
		$queryArr = array();
		$queryArr[] = $_SESSION['staffID'];
		$selectStatus = $this->selectTable2($sql, $queryArr);
		$view_flg3 = 0;
	...
	function process() {
		...
		switch ($mode) {
			...
			case "index":
				...
				// メインテンプレート
				$this->contents_tpl = $this->xml->getTemplate("top", "index");
