...
class Client_Tatekae_Index extends NK_Page {
	...
	function process() {
		...
		switch ($mode) {
			...
			case "index":
				...
				// メインテンプレート
				$this->contents_tpl = $this->xml->getTemplate("client", "tatekae_index");
		...
		*/
		...
		// 立替金一覧取得（count）
		$sql = $this->xml->getSql("client", "count_tatekae");
		if ($arrForm['cashAdvance'] != "") {
			$sql .= $this->xml->getSql("client", "select_tatekae_cashAdvance");
		}
		if ($arrForm['processStatus'] != "") {
			$sql .= $this->xml->getSql("client", "select_tatekae_processStatus");
		}
		if ($arrForm['numberOfPayment'] != "") {
			$sql .= $this->xml->getSql("client", "select_tatekae_numberOfPayment");
		}
		if ($arrForm['yearStart'] != "" && $arrForm['monthStart'] != "" && $arrForm['dayStart'] != "") {
			$sql .= $this->xml->getSql("client", "select_tatekae_startDate");
		}
		if ($arrForm['yearEnd'] != "" && $arrForm['monthEnd'] != "" && $arrForm['dayEnd'] != "") {
			$sql .= $this->xml->getSql("client", "select_tatekae_endDate");
		}
		...
		$queryArr[] = 1;
		if ($arrForm['cashAdvance'] != "") {
			$queryArr[] = $arrForm['cashAdvance'] ;
		}
		if ($arrForm['processStatus'] != "") {
			$queryArr[] = $arrForm['processStatus'];
		}
		if ($arrForm['numberOfPayment'] != "") {
			$queryArr[] = $arrForm['numberOfPayment'];
		}
		if ($arrForm['yearStart'] != "" && $arrForm['monthStart'] != "" && $arrForm['dayStart'] != "") {
			$queryArr[] = $arrForm['yearStart'].sprintf("%02d", $arrForm['monthStart']).sprintf("%02d", $arrForm['dayStart']);
		}
		if ($arrForm['yearEnd'] != "" && $arrForm['monthEnd'] != "" && $arrForm['dayEnd'] != "") {
			$queryArr[] = $arrForm['yearEnd'].sprintf("%02d", $arrForm['monthEnd']).sprintf("%02d", $arrForm['dayEnd']);
		}
		...
		// 取引先一覧取得
		$sql = $this->xml->getSql("client", "select_tatekae");
		if ($arrForm['cashAdvance'] != "") {
			$sql .= $this->xml->getSql("client", "select_tatekae_cashAdvance");
		}
		if ($arrForm['processStatus'] != "") {
		...
		$selectBase = $this->selectTable2($sql, $queryArr);
		$countSelect = count($selectBase);
