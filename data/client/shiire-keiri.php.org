...
class Client_Shiire_keiri extends NK_Page {
	...
	function init() {
		...
		// 権限チェック
		$sql = $this->xml->getSql("common", "status_purchaseAcconuntingBrowse");
		$queryArr = array();
		$queryArr[] = $_SESSION['staffID'];
		$selectStatus = $this->selectTable2($sql, $queryArr);
		$view_flg2 = 0;
		...
		// 権限チェック
		$sql = $this->xml->getSql("common", "status_purchaseAcconuntingEdit");
		$queryArr = array();
		$queryArr[] = $_SESSION['staffID'];
		$selectStatus = $this->selectTable2($sql, $queryArr);
		$view_flg3 = 0;
	...
	function process() {
		...
		switch ($mode) {
			...
			case "index":
				...
				// メインテンプレート
				$this->contents_tpl = $this->xml->getTemplate("client", "shiire_keiri");
			...
			case "newRegistBase":
				...
				if (count($arrForm['err_msg2']) == 0) {
					...
					$sql = $this->xml->getSql("client", "insert_client_shiire_keiri");
					$queryArr = array();
					foreach ($arrForm as $key => $value) {
						if (in_array($key, $nonRegArr) || preg_match('/^FN_/', $key)) {
						} else {
			...
			case "editRegistBase":
				...
				if (count($arrForm['err_msg2']) == 0) {
					...
					$sql = $this->xml->getSql("client", "update_client_shiire_keiri");
					$queryArr = array();
					foreach ($arrForm as $key => $value) {
						if (in_array($key, $nonRegArr) || preg_match('/^FN_/', $key)) {
						} else {
		...
		if (count($arrForm['err_msg']) == 0) {
			...
			// 基本取得
			$sql = $this->xml->getSql("client", "select_client_shiire_keiri");
			$queryArr = array();
			$queryArr[] = $codeNumber;
			$queryArr[] = $officeCode;
			$queryArr[] = 1;
			...
			if (count($selectBaseTemp) > 0 && !$editFlgBase) {
