					...
					<!--{if $view_flg2 == 1}-->
						<table class="tableA">
							<tr>
								...
								<td>
									<input type="text" name="officialName" class="w335" value="<!--{$arrForm.officialName}-->">
								</td>
								<th class="w70">コードNo</th>
								<td>
									<input type="text" id="codeNumberText" class="w145" name="codeNumber" value="<!--{$arrForm.codeNumber}-->">
								</td>
								<th>読みカナ分類</th>
								<td>
									<select name="constructionStates" class="w154">
										<option value="ア"<!--{if $arrForm.constructionStates == "ア"}--> selected<!--{/if}-->>ア</option>
										<option value="カ"<!--{if $arrForm.constructionStates == "カ"}--> selected<!--{/if}-->>カ</option>
										<option value="サ"<!--{if $arrForm.constructionStates == "サ"}--> selected<!--{/if}-->>サ</option>
										<option value="タ"<!--{if $arrForm.constructionStates == "タ"}--> selected<!--{/if}-->>タ</option>
										<option value="ナ"<!--{if $arrForm.constructionStates == "ナ"}--> selected<!--{/if}-->>ナ</option>
										<option value="ハ"<!--{if $arrForm.constructionStates == "ハ"}--> selected<!--{/if}-->>ハ</option>
										<option value="マ"<!--{if $arrForm.constructionStates == "マ"}--> selected<!--{/if}-->>マ</option>
										<option value="ヤ"<!--{if $arrForm.constructionStates == "ヤ"}--> selected<!--{/if}-->>ヤ</option>
										<option value="ラ"<!--{if $arrForm.constructionStates == "ラ"}--> selected<!--{/if}-->>ラ</option>
										<option value="ワ"<!--{if $arrForm.constructionStates == "ワ"}--> selected<!--{/if}-->>ワ</option>
									</select>
								</td>
							</tr>
						</table>
						<table class="tableA">
							<tr>
								...
								<td class="w238"><input type="text" name="clientNameInSystem" class="w234" value="<!--{$arrForm.clientNameInSystem}-->"></td>
								<th class="w45">備考</th>
								<td colspan="3"><input type="text" name="note" class="w100p" value="<!--{$arrForm.note}-->"></td>
							</tr>
						</table>
						<table class="tableA">
							<tr>
						...
						<table class="tableB">
							<tbody>
								<tr>
									...
									<td class="w75 txtCenter"><!--{$value.regist_name}--><br><!--{$value.update_name}--></td>
